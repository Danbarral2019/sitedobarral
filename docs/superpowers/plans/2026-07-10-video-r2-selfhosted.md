...
- Test: `app/api/admin/videos/confirm/__tests__/route.test.ts`
...
- [ ] **Step 1: Escrever o teste que falha (gate admin + validações + gravação)**

```typescript
// app/api/admin/videos/confirm/__tests__/route.test.ts
// @vitest-environment node
import { describe, it, expect, vi, beforeEach } from 'vitest';
...
Run: `npm run test:run -- app/api/admin/videos/confirm`
Expected: FAIL (rota não existe).
...
Run: `npm run test:run -- app/api/admin/videos/confirm`
Expected: PASS (5 testes). (Confirmar que `CacheInvalidation.courseVideos` existe — já usado em `app/api/admin/course-videos/route.ts:59`.)

- [ ] **Step 5: Commit**

```bash
git add app/api/admin/videos/confirm
