/**
 * Lista todas as Instruções Normativas no banco e roda validateActContent
 * em cada uma. Salva resultado em docs/audits/INS-AUDIT-<data>.json
 */
...
async function main() {
  const acts = await prisma.legislativeAct.findMany({
    where: {
      OR: [{ type: 'in' }, { fullNumber: { startsWith: 'IN ' } }],
    },
    orderBy: [{ year: 'desc' }, { number: 'desc' }],
  });

  console.log(`Encontradas ${acts.length} instruções normativas no banco.\n`);
  ...
  const outFile = join(process.cwd(), 'docs', 'audits', `${today}-ins-audit-db.json`);
  ...
  console.log(`📊 Resumo:`);
  console.log(`   Total INs:       ${acts.length}`);
  console.log(`   ✅ OK (sem err/warn): ${okCount}`);
  console.log(`   🚫 Com erros:     ${errorCount}`);
  console.log(`   ⚠️  Com warnings:  ${warningCount}`);
  console.log();
  console.log(`💾 Salvo em ${outFile}`);

  // Detalhe top problemas
  ...
  if (warned.length) {
    console.log(`\n⚠️  INs com warnings (${warned.length}):`);
    for (const e of warned.slice(0, 30)) {
      console.log(`   ${e.fullNumber}: ${e.validation.warnings.map((w) => w.slice(0, 80)).join('; ')}`);
    }
    if (warned.length > 30) console.log(`   ... e mais ${warned.length - 30}`);
  }
