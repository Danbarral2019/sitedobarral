// Gerador da folha de calibração de teses destiladas (Fase 2-A da rede de precedentes do TCU).
//
// Lê o JSON produzido pelo probe de destilação (docs/audits/2026-07-19-probe-teses-tcu.json,
// formato { geradoEm, casos: TeseDestilada[], dossies: DossieUso[] }) e gera um HTML
// autocontido (folha de calibração) para o Daniel julgar cada tese frente aos trechos-fonte
// reais que a sustentam. O HTML é escrito no scratchpad; a publicação como Artifact é feita
// à parte pelo controlador da tarefa.
//
// Uso: node scripts/build-folha-teses.mjs
...
const SCRATCHPAD =
  'C:/Users/User/AppData/Local/Temp/claude/C--Users-User/a71670c1-1ea7-46f6-a59a-8b82fcedb85e/scratchpad';

const JSON_PATH = path.join(REPO_ROOT, 'docs', 'audits', '2026-07-19-probe-teses-tcu.json');
const OUT_PATH = path.join(SCRATCHPAD, 'folha-teses-tcu.html');
...
// Ordena por contagem.noVoto desc — o sinal mais forte de leading case (razão de decidir).
cards.sort((a, b) => b.contagem.noVoto - a.contagem.noVoto);

const DATA = JSON.stringify(cards);
const GERADO_EM = src.geradoEm || '';

const html = `<title>Calibração — Teses do TCU (Fase 2-A)</title>
<style>
:root{
...
<header>
  <div class="head-in">
    <div class="eyebrow">Rede de precedentes · Fase 2-A · probe de destilação de teses</div>
...
function renderTeseBlock(t, idx, total) {
...
function renderCard(d, order) {
  ...
  if (!d.teses || !d.teses.length) {
    ...
    for (let i = 0; i < d.teses.length; i++) {
      html += renderTeseBlock(d.teses[i], i, d.teses.length);
    }
    html += '</div>';
  }
...
document.getElementById('btnExport').addEventListener('click', function () {
  let lines = [];
  lines.push('CALIBRAÇÃO DE TESES — Rede de precedentes TCU (Fase 2-A)');
  lines.push('');
...
console.log('OK — folha-teses-tcu.html gerada, ' + cards.length + ' cards, ' + html.length + ' bytes -> ' + OUT_PATH);
