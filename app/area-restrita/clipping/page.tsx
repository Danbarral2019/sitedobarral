...
export default async function ClippingArchivePage({ searchParams }: PageProps) {
 ...
 <div className="bg-surface-raised border border-border-subtle rounded-md p-4 text-sm text-amber-accent-deep">
 ...
 className="flex-1 border border-border-strong rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-amber-accent"
 />
 <button
 type="submit"
 className="bg-brand-700 text-surface-page text-sm font-semibold px-4 py-2 rounded-md hover:bg-brand-800"
 >
 Buscar
 </button>
 {query ? (
 ...
 return (
 <li key={dateParam} className="bg-surface-page border border-border-subtle rounded-md p-4">
 <Link
 ...
 {hit.snippet ? (
 <p className="text-sm text-ink-secondary mt-2 line-clamp-2">{hit.snippet}…</p>
 ) : null}
 </li>
 );
 })}
 ...
 <p className="text-sm text-ink-secondary mt-2 line-clamp-2">{e.preview}</p>
 ) : null}
 </li>
 );
 })}
