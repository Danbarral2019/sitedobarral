    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-ink-muted">Carregando certificados...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <div className="flex items-center gap-4 mb-4">
            <div className="w-16 h-16 bg-amber-accent rounded-full flex items-center justify-center border border-border-subtle">
        ...
        {certificates.length === 0 ? (
          <div className="bg-white rounded-[6px] p-12 text-center border-2 border-border-subtle">
            <Award className="w-16 h-16 text-ink-muted mx-auto mb-4" />
          ...
          <div className="space-y-4">
            {certificates.map((cert) => {
              ...
              return (
                <div
                  key={cert.id}
                  className={`rounded-[6px] p-6 transition-shadow ${
                    revoked
                      ? 'bg-surface-raised border-2 border-dashed border-red-200 opacity-80'
                      : 'bg-amber-accent-soft border border-amber-accent-soft hover:'
                  }`}
                >
                  <div className="flex items-start gap-4">
                    <div
                      className={`w-12 h-12 rounded-[6px] flex items-center justify-center flex-shrink-0 ${
                        revoked ? 'bg-red-100 text-red-600' : 'bg-amber-accent-soft text-amber-accent-deep'
                      }`}
                    >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {revoked && cert.revokeReason && (
                        <div className="bg-red-50 border border-red-200 rounded-[6px] p-3 mb-4 text-sm text-red-900">
                      ...
                      <div className="flex flex-wrap gap-3">
                        ...
                        <a
                          ...
                          className={`inline-flex items-center gap-2 border px-4 py-2 rounded-[6px] font-medium text-sm transition-colors ${
                            revoked
                              ? 'border-border-subtle text-ink-muted hover:bg-surface-deep'
                              : 'border-amber-accent text-amber-accent-deep hover:bg-amber-accent-soft'
                          }`}
                        >
