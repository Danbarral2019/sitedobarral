                            ...
                            {docs.map((doc) => (
                              <div key={doc.id} className="space-y-0">
                                <button
                                  ...
                                  className={`w-full text-left p-3 rounded-lg transition-colors border group ${getTierCardStyle(tier)} ${doc.summary && !isLiteralSourceCategory(doc.category) && expandedSummaryId === doc.id ? 'rounded-b-none border-b-0' : ''}`}
                                >
                                  <div className="flex items-start justify-between gap-3">
                                    ...
                                    <div className="flex items-center gap-1 flex-shrink-0">
                                      {/* Botão "Ver resumo": exibido apenas para não-literais com summary IA. Ver lib/literal-sources.ts. */}
                                      {doc.summary && !isLiteralSourceCategory(doc.category) && (
                                        <button
                                          onClick={(e) => {
                                            e.stopPropagation();
                                ...
                                {/* Painel expandido de summary IA — só não-literais */}
                                {doc.summary && !isLiteralSourceCategory(doc.category) && expandedSummaryId === doc.id && (
