...
const TYPE_COLORS: Record<string, string> = {
 'decreto': 'bg-surface-deep text-ink-primary',
 'portaria': 'bg-surface-deep text-ink-primary',
 'in': 'bg-surface-deep text-ink-primary',
 'ordem-servico': 'bg-amber-accent-soft text-amber-accent-deep',
 'lei': 'bg-surface-deep text-semantic-error',
 'medida-provisoria': 'bg-surface-deep text-ink-primary',
...
export default function FavoritosPage() {
 ...
 <p className="text-ink-secondary">Carregando favoritos...</p>
 </div>
 </div>
 );
 }
 ...
 className={`px-4 py-2 rounded-[3px] font-medium transition-colors flex items-center gap-2 ${
 activeTab === 'documents'
 ? 'bg-brand-600 text-surface-page'
 : 'bg-surface-page text-ink-secondary hover:bg-surface-deep border border-border-subtle'
 }`}
 >
 ...
 <Heart className="w-16 h-16 text-border-strong mx-auto mb-4" />
 ...
 {favorites.length === 0 ? (
 <div className="bg-surface-page rounded-md p-8 text-center border border-border-subtle">
 <FileText className="w-12 h-12 text-border-strong mx-auto mb-3" />
 <p className="text-ink-secondary">Nenhum documento favoritado ainda.</p>
 </div>
 ) : (
 groupedFavorites.map((group) => (
 <div key={group.courseId} className="bg-surface-page rounded-md border border-border-subtle overflow-hidden">
 {/* Header do Curso */}
 <div className="bg-surface-raised text-ink-primary p-6">
 ...
 <div
 key={fav.id}
 className="flex items-start gap-4 p-4 bg-surface-raised border border-border-subtle rounded-md hover:border-border-strong transition-all"
 >
 {/* Ícone */}
 <div className="w-12 h-12 bg-surface-deep rounded-[3px] flex items-center justify-center flex-shrink-0 text-brand-600">
 ...
 <p className="text-sm text-ink-secondary mb-2 line-clamp-2">
 {fav.document.description}
 </p>
 )}
 <div className="flex items-center gap-3 flex-wrap">
 <span className="inline-block px-3 py-1 bg-surface-deep text-ink-primary rounded-full text-xs font-medium">
 ...
 fav.annotation
 ? 'text-amber-accent bg-surface-raised hover:bg-amber-accent-soft'
 : 'text-ink-muted hover:text-amber-accent hover:bg-surface-raised'
 }`}
 ...
 className="p-2 text-brand-600 hover:bg-surface-raised rounded-[3px] transition-colors"
 ...
 className="p-2 text-semantic-error hover:bg-surface-raised rounded-[3px] transition-colors disabled:opacity-50"
 title="Remover dos favoritos"
 >
 {removingId === fav.documentId ? (
 <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-semantic-error"></div>
 ) : (
 ...
 <label className="text-xs font-semibold text-ink-secondary">Anotacao de estudo</label>
 ...
 className="w-full p-3 text-sm border border-border-strong rounded-[3px] resize-y min-h-[80px] max-h-[200px] focus:outline-none focus:ring-2 focus:ring-amber-accent focus:border-transparent bg-surface-page"
 maxLength={5000}
 />
 ...
 <button
 onClick={() => toggleAnnotation(fav.id, fav.annotation)}
 className="mt-2 w-full text-left p-2 bg-surface-raised border border-border-subtle rounded-[3px] text-sm text-amber-accent-deep hover:bg-amber-accent-soft transition-colors"
 >
 <span className="font-medium text-xs text-amber-accent block mb-0.5">Anotacao:</span>
 ...
 <p className="text-ink-secondary">Nenhuma Orientação Normativa favoritada ainda.</p>
 <Link
 ...
 <div className="flex items-start justify-between gap-4">
 <div className="flex-1">
 <span className="inline-block px-2 py-0.5 bg-surface-deep text-ink-primary rounded text-xs font-medium mb-1">
 ...
 <div className="bg-surface-page rounded-md p-8 text-center border border-border-subtle">
 <Scale className="w-12 h-12 text-border-strong mx-auto mb-3" />
 <p className="text-ink-secondary">Nenhum ato normativo favoritado ainda.</p>
 <Link
 href="/area-restrita/lei-comentada"
 className="inline-block mt-4 text-brand-600 hover:text-brand-700 font-medium"
 >
 ...
 const typeColor = TYPE_COLORS[act.type] || 'bg-surface-deep text-ink-primary';
 ...
 <p className="text-xs text-ink-secondary line-clamp-2 mb-2">{act.ementa}</p>
