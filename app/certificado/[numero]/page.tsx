  ...
  return (
    <main className="min-h-screen bg-surface-raised">
      <div className="max-w-4xl mx-auto px-4 py-12">
        {/* Status header */}
        {revoked ? (
          <div className="bg-red-600 text-white rounded-t-[6px] px-8 py-5 flex items-center justify-center gap-3 border border-border-subtle">
          ...
          <div className="bg-emerald-600 text-white rounded-t-[6px] px-8 py-5 flex items-center justify-center gap-3 border border-border-subtle">
            <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden>
        ...
        {/* Card principal */}
        <div className="bg-white rounded-b-[6px] overflow-hidden border border-border-subtle">
          {/* Hero navy */}
          <div className="bg-[#20364e] to-[#142232] px-8 py-12 text-center text-white relative">
            <div className="absolute top-4 right-4 text-xs uppercase tracking-widest text-ink-muted/60">
              Autenticação
            </div>
            <p className="text-xs uppercase tracking-[0.25em] text-ink-muted mb-3">Prof. Daniel Barral</p>
            <p className="text-sm font-light text-ink-muted mb-2">Certifica que</p>
          ...
          {/* Detalhes */}
          <div className="grid md:grid-cols-2 gap-0 border-t border-border-subtle">
            <div className="px-8 py-6 border-b md:border-b-0 md:border-r border-border-subtle">
              <dl className="space-y-4 text-sm">
                <div>
                  <dt className="text-xs uppercase tracking-wide text-ink-muted font-medium">Data de emissão</dt>
                  <dd className="text-base text-ink-primary font-semibold mt-1">{issuedDate}</dd>
                </div>
                <div>
                  <dt className="text-xs uppercase tracking-wide text-ink-muted font-medium">Número do certificado</dt>
                  <dd className="text-base font-mono text-ink-primary font-semibold mt-1">
                ...
                {certificate.issuedById && !revoked ? (
                  <div>
                    <dt className="text-xs uppercase tracking-wide text-ink-muted font-medium">Modo de emissão</dt>
                    <dd className="text-sm text-ink-secondary mt-1">Emissão administrativa</dd>
                  </div>
                ) : null}
              </dl>
            </div>
            <div className="px-8 py-6 flex flex-col items-center justify-center bg-surface-raised">
              {qrDataUrl ? (
                ...
                <div className="w-[180px] h-[180px] bg-surface-deep rounded-md" aria-hidden />
              )}
              <p className="text-xs text-ink-muted mt-3 text-center max-w-[200px]">
          ...
          {/* CTAs */}
          <div className="px-8 py-6 border-t border-border-subtle bg-white">
            {revoked ? (
              <div className="bg-red-50 border border-red-200 rounded-[6px] px-4 py-3 text-sm text-red-900 text-center">
...
function NotFoundView({ numero }: { numero: string }) {
  return (
    <main className="min-h-screen bg-surface-raised flex items-center justify-center px-4">
      <div className="max-w-md w-full text-center bg-white rounded-[6px] border border-border-subtle p-8">
        <div className="w-16 h-16 mx-auto mb-4 rounded-full bg-red-100 flex items-center justify-center">
          <svg className="w-8 h-8 text-red-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
          </svg>
        </div>
        <h1 className="text-xl font-bold text-ink-primary mb-2">Certificado não encontrado</h1>
