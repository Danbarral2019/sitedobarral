...
export default function EditPublicationPage({ params }: EditPublicationPageProps) {
  ...
  const loadPublication = async (id: string) => {
    try {
      ...
      setFormData({
        ...
        content: pub.content || '',
        author: pub.author || 'Prof. Daniel Barral',
        publishedAt: pub.publishedAt ? new Date(pub.publishedAt).toISOString().split('T')[0] : new Date().toISOString().split('T')[0],
        ...
        journal: pub.journal || '',
        eventDate: pub.eventDate ? new Date(pub.eventDate).toISOString().split('T')[0] : '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || 'Erro ao atualizar publicação');
      }

      success('Publicação atualizada!', 'As alterações foram salvas com sucesso.');
      router.push('/admin/publicacoes');
    } catch (error) {
  ...
  const handleDelete = async () => {
    if (!confirm('Tem certeza que deseja deletar esta publicação? Esta ação não pode ser desfeita.')) {
      return;
    }
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="mb-8">
            <div className="flex items-center justify-between">
              <h1 className="text-4xl font-bold text-gray-900">Editar Publicação</h1>
              <Link
