    ...
    <div className="min-h-screen bg-surface-raised">
      ...
      <div className="bg-white border-b">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <nav className="flex items-center space-x-2 text-sm text-ink-muted">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          ...
          <div className="bg-white rounded-[6px] border p-8 mb-6">
            ...
            <div className="flex flex-wrap gap-2">
              {term.leiArticles.map((article: string) => (
                <span
                  key={article}
                  className="inline-flex items-center px-3 py-1.5 rounded-md text-sm font-medium bg-surface-deep text-ink-secondary"
                >
                  Art. {article}
                </span>
              ))}
        ...
        {term.relatedDocuments && term.relatedDocuments.length > 0 && (
          <div className="bg-white rounded-[6px] border p-6 mb-6">
            ...
            <div className="space-y-3">
              {term.relatedDocuments.map((doc: RelatedDocument) => (
                <div
                  key={doc.id}
                  className="p-4 bg-surface-raised rounded-[6px] hover:bg-surface-deep transition-colors"
                >
                  <h3 className="font-semibold text-ink-primary mb-1">{doc.title}</h3>
                  {doc.description && (
                    <p className="text-sm text-ink-muted mb-2">{doc.description}</p>
                  )}
        ...
        {term.relatedTerms && term.relatedTerms.length > 0 && (
          <div className="bg-white rounded-[6px] border p-6">
            <h2 className="text-xl font-bold text-ink-primary mb-4">
              Termos Relacionados
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {term.relatedTerms.map((relatedTerm: RelatedTerm) => (
                <Link
                  ...
                  className="p-4 bg-surface-raised rounded-[6px] hover:bg-brand-50 hover:border-brand-300 border border-border-subtle transition-all"
                >
                  ...
                  {relatedTerm.category && (
                    <span className="inline-block mt-2 text-xs px-2 py-1 bg-white rounded text-ink-muted">
                      {relatedTerm.category}
                    </span>
                  )}
                </Link>
