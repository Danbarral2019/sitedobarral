...
import type { CursoVendavel } from '@/lib/lms/cursos-vendaveis';

interface PlanosClientProps {
  /**
   * Cursos oferecidos na escolha do Básico. Vem do servidor já filtrado por
   * quem tem aula publicada — ver lib/lms/cursos-vendaveis.ts.
   */
  cursosVendaveis: CursoVendavel[];
}
...
export default function PlanosClient({ cursosVendaveis }: PlanosClientProps) {
  const router = useRouter();
  const { user, isLoading } = useAuth();
  const [selectedCourse, setSelectedCourse] = useState(cursosVendaveis[0]?.id ?? '');
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 gap-8 mb-16">
          ...
          <div className="bg-white rounded-[6px] p-8 border border-border-subtle flex flex-col">
            ...
            {/* Seletor de curso. Só entram cursos com aula publicada — se não
                houver nenhum, o Básico não tem o que vender e o cartão avisa
                em vez de oferecer um seletor vazio. */}
            {cursosVendaveis.length === 0 ? (
              <div className="mb-6 rounded-[6px] border border-border-subtle bg-surface-deep p-4">
                <p className="text-sm text-ink-secondary">
                  Nenhum curso está aberto para matrícula avulsa no momento. O plano
                  Premium segue disponível, com o acervo restrito e o assistente de IA.
                </p>
              </div>
            ) : (
            <div className="mb-6">
            ...
            <button
              ...
              disabled={loading !== null || isLoading || cursosVendaveis.length === 0}
            ...
            <ul className="space-y-4 mb-8 flex-grow">
              <li className="flex items-start gap-3">
                ...
                <span className="text-white">
                  {/* Conta os cursos com aula publicada, não os sete do
                      catálogo: anunciar quatro salas vazias é o mesmo
                      problema que o seletor do Básico já evita. */}
                  Acesso a todos os {cursosVendaveis.length} cursos
                </span>
              </li>
              <li className="flex items-start gap-3">
                <CheckIcon className="w-5 h-5 text-green-300 mt-0.5 flex-shrink-0" />
