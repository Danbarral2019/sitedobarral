 ...
 */
export const PUT = withAdminApi<{ id: string; questionId: string }>(async (request, ctx) => {
  const { id: quizId, questionId } = ctx.params;
 ...
 */
export const DELETE = withAdminApi<{ id: string; questionId: string }>(async (request, ctx) => {
  const { id: quizId, questionId } = ctx.params;
