...
vi.mock('@/lib/lms/query-timing', () => ({
  withTiming: vi.fn(async (_label: string, operation: () => Promise<unknown>) => operation()),
}));
vi.mock('@/lib/lms/analytics-queries', () => ({
  getPassedQuizIds: mocks.getPassedQuizIds,
}));
vi.mock('@/lib/logger', () => ({
...
function makeRequest(): NextRequest {
  const request = new NextRequest('http://localhost/api/area-restrita/courses/course-1/modules');
...
describe('GET /api/area-restrita/courses/[courseId]/modules - acesso ao curso', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    ...
    mocks.getPassedQuizIds.mockResolvedValue(new Set<string>());
  });
