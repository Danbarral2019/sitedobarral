...
import { isArtigoMalFormatado, revokerFromNote, contentMostraAlteracao } from '@/lib/data-health/checks';
...
export async function GET(request: NextRequest) {
  ...
  try {
    await withCronTelemetry('data-health', async () => {
      const [revokedFP, artigoFmt] = await Promise.all([
      ...
      responseBody = {
        ...
        checks: { revokedFalsePositives: revokedFP, artigoFormat: artigoFmt },
      };
      return {
        itemsFound: revokedFP.count + artigoFmt.count,
        itemsError: revokedFP.count + artigoFmt.count,
        metadata: { revokedFP: revokedFP.count, artigoFmt: artigoFmt.count },
      };
    });
