...
export function LegislativeActCard({
  ...
  return (
    <article
      id={act.id}
      className={`bg-white border-2 rounded-xl overflow-hidden transition-all border-gray-200 ${theme.cardHoverBorder}`}
    >
      <div className="p-6">
        <div className="flex items-start justify-between gap-4 mb-4">
          <div className="flex-1">
            <div className="flex flex-wrap items-center gap-2 mb-3">
              <span className={`px-3 py-1 text-sm font-bold rounded-lg border-2 ${getTypeColor(act.type)}`} title={typeTooltip}>
      ...
      {isExpanded && (
        <div className="border-t-2 border-gray-200 bg-gray-50 p-6">
          {act.summary && (
            <div className={`mb-6 rounded-xl overflow-hidden shadow-sm ${theme.summaryGradient}`}>
          ...
          <div className="flex flex-wrap gap-3">
            {act.officialUrl && (
              <a
                ...
                className={`flex items-center gap-2 px-4 py-2.5 text-white rounded-lg transition-colors font-semibold ${theme.primaryActionBg}`}
              >
