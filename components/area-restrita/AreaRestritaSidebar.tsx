...
export function AreaRestritaSidebar() {
 ...
 className="fixed left-0 top-0 z-[70] flex h-screen w-[85%] max-w-sm flex-col border-r border-border-subtle bg-surface-page"
 >
 ...
 aria-label="Menu de navegação"
 className={cn(
 'fixed left-0 top-0 z-40 hidden h-screen w-72 flex-col border-r border-border-subtle bg-surface-page transition-transform duration-200',
 ...
 <button
 onClick={toggleDesktop}
 className="fixed left-4 top-20 z-40 hidden items-center gap-1 rounded-[3px] border border-border-subtle bg-surface-page px-3 py-2 text-xs font-medium text-ink-secondary hover:border-brand-300 hover:text-brand-700 lg:inline-flex"
...
export function SidebarMobileTrigger({
 ...
 className="flex flex-1 flex-col items-center justify-center h-full text-ink-secondary hover:text-brand-600 focus-visible:ring-2 focus-visible:ring-amber-accent focus-visible:ring-offset-2"
 aria-label="Abrir menu"
 >
 ...
 <button
 onClick={toggleMobile}
 className="flex items-center gap-1.5 px-3 py-2 text-ink-secondary hover:text-brand-700 hover:bg-brand-50 rounded-[3px] transition-colors font-medium text-sm lg:hidden focus-visible:ring-2 focus-visible:ring-amber-accent focus-visible:ring-offset-2"
 aria-label="Abrir menu"
 >
...
function SidebarLink({
 ...
 const className = cn(
 'flex items-start gap-3 rounded-[3px] px-3 py-2 text-sm transition',
 active
 ? 'bg-brand-50 text-brand-900'
 : 'text-ink-primary hover:bg-brand-50 hover:text-brand-800',
 );
 const content = (
 <>
 <span
 ...
 active ? 'bg-brand-600 text-surface-page' : 'bg-brand-50 text-brand-700',
 )}
 >
 ...
 {item.badge === 'novo' && (
 <span className="ml-2 shrink-0 self-center rounded-full bg-surface-deep px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-ink-primary">
 Novo
 </span>
 )}
 </>
