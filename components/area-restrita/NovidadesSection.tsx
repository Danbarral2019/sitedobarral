...
export default function NovidadesSection({ onDocumentClick }: NovidadesSectionProps) {
 ...
 if (isLoading) {
 return (
 <div className="mb-6 bg-surface-page rounded-md border border-border-subtle p-4">
 ...
 return (
 <div className="mb-6 bg-surface-page rounded-md border border-border-subtle overflow-hidden">
 {/* Header */}
 <div className="px-4 py-3 bg-surface-raised border-b border-border-subtle flex items-center justify-between">
 <div className="flex items-center gap-2">
 <div className="p-1.5 bg-surface-deep rounded-[3px]">
 <Bell className="w-4 h-4 text-ink-secondary" />
 </div>
 <h3 className="text-sm font-bold text-ink-primary">Novidades</h3>
 {timelineItems.length > 0 && (
 <span className="text-[10px] font-bold bg-brand-600 text-surface-page px-1.5 py-0.5 rounded-full">
 ...
 : item.type === 'update'
 ? 'text-amber-accent bg-surface-raised'
 : item.type === 'tribunal'
 ? 'text-ink-secondary bg-surface-raised'
 ...
 <div className={`p-1.5 rounded-[3px] flex-shrink-0 ${colorClass}`}>
 <IconComp className="w-3.5 h-3.5" />
 </div>
 <div className="flex-1 min-w-0">
 <p className={`text-sm font-medium text-ink-primary truncate ${isClickable ? 'group-hover:text-brand-600' : ''} transition-colors`}>
 {item.title}
 </p>
 {item.subtitle && (
 ...
 {isClickable && (
 <ChevronRight className="w-3.5 h-3.5 text-border-strong flex-shrink-0" />
 )}
 </div>
 );
