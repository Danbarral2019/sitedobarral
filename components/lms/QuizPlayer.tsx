...
export default function QuizPlayer({ lessonId, onQuizPass }: QuizPlayerProps) {
 ...
 <ClipboardCheck className="w-5 h-5 text-ink-secondary" />
 ...
 {submittedScore.passed ? (
 <Trophy className="w-6 h-6 text-ink-secondary" />
 ) : (
 <XCircle className="w-6 h-6 text-semantic-error" />
 )}
 <div>
 <p className={`font-bold text-lg ${submittedScore.passed ? 'text-ink-secondary' : 'text-semantic-error'}`}>
 ...
 return (
 <div key={q.id} className="bg-surface-page rounded-[3px] p-4 border border-border-subtle">
 <div className="flex items-start gap-2 mb-2">
 {result?.correct ? (
 <CheckCircle className="w-4 h-4 text-ink-secondary mt-0.5 flex-shrink-0" />
 ) : (
 <XCircle className="w-4 h-4 text-semantic-error mt-0.5 flex-shrink-0" />
 )}
 ...
 isCorrect
 ? 'bg-surface-raised text-ink-secondary font-medium'
 : isSelected
 ? 'bg-surface-raised text-semantic-error'
 : 'text-ink-muted'
 }`}
 >
 {opt.text}
 ...
 {quiz.description && (
 <p className="text-sm text-ink-secondary">{quiz.description}</p>
 )}
 ...
 {hasPassed && (
 <div className="flex items-center gap-2 text-ink-secondary bg-surface-raised rounded-[3px] px-3 py-2">
 ...
 <button
 onClick={handleStart}
 className="inline-flex items-center gap-2 bg-brand-600 text-surface-page px-5 py-2.5 rounded-[3px] font-medium text-sm hover:bg-brand-800 transition-colors"
 >
 ...
 {!canRetry && !hasPassed && (
 <div className="flex items-center gap-2 text-amber-accent bg-surface-raised rounded-[3px] px-3 py-2 text-sm">
 ...
 timeRemaining < 60 ? 'text-semantic-error' : 'text-ink-secondary'
 }`}>
 <Clock className="w-4 h-4" />
 ...
 <div
 className="h-full bg-surface-raised0 rounded-full transition-all duration-300"
 ...
 <div key={question.id} className="border border-border-subtle rounded-md p-5">
 <p className="text-sm font-medium text-ink-primary mb-3">
 <span className="text-ink-secondary font-bold">{idx + 1}.</span>{' '}
 {question.text}
 {question.points > 1 && (
 ...
 className={`w-full text-left px-4 py-3 rounded-[3px] border text-sm transition-all ${
 isSelected
 ? 'border-brand-600 bg-surface-raised text-ink-secondary font-medium'
 : 'border-border-subtle hover:border-border-strong hover:bg-surface-raised/30 text-ink-secondary'
 }`}
 >
 {option.text}
 </button>
 ...
 className="inline-flex items-center gap-2 bg-brand-600 text-surface-page px-6 py-2.5 rounded-[3px] font-medium text-sm hover:bg-brand-800 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
 >
 {isSubmitting ? (
 <>
 ...
 {error && (
 <p className="mt-3 text-sm text-semantic-error">{error}</p>
 )}
 </div>
 );
}
