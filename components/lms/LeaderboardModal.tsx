...
export default function LeaderboardModal({ courseId, onClose }: LeaderboardModalProps) {
 ...
 <button
 onClick={onClose}
 className="p-1 text-surface-page/80 hover:text-surface-page transition-colors"
 >
 <X className="w-5 h-5" />
 </button>
 </div>
 ...
 <Trophy className="w-10 h-10 text-ink-muted mx-auto mb-3" />
 <p className="text-sm text-ink-muted">Nenhum aluno no ranking ainda.</p>
 <p className="text-xs text-ink-muted mt-1">Complete aulas e quizzes para ganhar XP!</p>
 </div>
 ) : (
 ...
 entry.isCurrentUser ? 'bg-surface-raised' : ''
 }`}
 >
 {/* Position */}
 <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 text-sm font-bold ${
 entry.position === 1 ? 'bg-amber-accent-soft text-amber-accent-deep' :
 entry.position === 2 ? 'bg-surface-deep text-ink-secondary' :
 entry.position === 3 ? 'bg-surface-deep text-ink-secondary' :
 ...
 {entry.name}
 {entry.isCurrentUser && <span className="text-xs text-ink-secondary ml-1">(voce)</span>}
 </p>
 ...
 {entry.badgeCount > 0 && (
 <span className="flex items-center gap-0.5">
 <Award className="w-3 h-3 text-amber-accent" />
 {entry.badgeCount}
 </span>
 )}
 </div>
 ...
 <div className="px-5 py-3 bg-surface-raised text-center">
 ...
 onClick={toggleOptIn}
 disabled={isToggling}
 className={`flex items-center gap-1.5 px-3 py-1.5 rounded-[3px] text-xs font-semibold transition-colors ${
 showOnLeaderboard
 ? 'bg-surface-deep text-brand-700 hover:bg-surface-deep'
