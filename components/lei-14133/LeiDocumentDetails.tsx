...
export function LeiDocumentDetails({ documentId, documentType = 'document' }: LeiDocumentDetailsProps) {
  ...
  if (loading) {
    return (
      <div className="bg-surface-raised p-8 rounded-lg border-t border-border-subtle">
        <Loader2 className="w-8 h-8 animate-spin text-brand-600 mx-auto" />
        <p className="text-center text-ink-secondary mt-3">Carregando detalhes...</p>
      </div>
    );
  }
  ...
  if (error || !document) {
    return (
      <div className="bg-surface-raised p-6 rounded-lg border-t border-border-subtle">
        <AlertCircle className="w-8 h-8 text-semantic-error mx-auto mb-2" />
        <p className="text-center text-semantic-error">{error || 'Documento não encontrado'}</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-surface-raised p-6 rounded-md border-t border-border-subtle space-y-4">
      {isLiteral && document.description && (
        <div className="bg-surface-raised border-l-4 border-brand-600 p-4 rounded-r-lg">
          <div className="flex items-center gap-2 mb-2">
            ...
            <h4 className="font-bold text-ink-primary text-sm">Texto do Enunciado</h4>
          </div>
          <p className="text-ink-primary text-sm leading-relaxed whitespace-pre-line">{document.description}</p>
        </div>
      )}
        ...
        <div className="bg-surface-raised border-l-4 border-brand-600 p-4 rounded-r-lg">
          ...
          <div className="space-y-2">
            {normalizeTextContent(document.summary).map((p, i) => (
              <p key={i} className="text-ink-primary text-sm leading-relaxed">{p}</p>
            ))}
          </div>
        </div>
      )}
      ...
      {keyPoints.length > 0 && (
        <div className="bg-surface-raised border-l-4 border-brand-600 p-4 rounded-r-lg">
          <div className="flex items-center gap-2 mb-2">
            ...
            <h4 className="font-bold text-ink-primary text-sm">Pontos-Chave</h4>
          </div>
      ...
      {effectivePracticalUse && (
        <div className="bg-surface-raised border-l-4 border-brand-600 p-4 rounded-r-lg">
          <div className="flex items-center gap-2 mb-2">
            <Lightbulb className="w-4 h-4 text-ink-secondary" />
            <h4 className="font-bold text-ink-primary text-sm">Aplicação Prática</h4>
          </div>
          <div className="space-y-2">
      ...
      {effectivePublicNotes && (
        <div className="bg-surface-raised border-l-4 border-amber-accent p-4 rounded-r-lg">
          <div className="flex items-center gap-2 mb-2">
            <BookOpen className="w-4 h-4 text-amber-accent" />
          ...
          <div className="space-y-2">
            {normalizeTextContent(effectivePublicNotes).map((p, i) => (
              <p key={i} className="text-ink-primary text-sm leading-relaxed italic">{p}</p>
            ))}
          </div>
        </div>
      )}
      ...
      {!isLiteral && document.description && !document.summary && (
        <div className="bg-surface-raised border-l-4 border-brand-600 p-4 rounded-r-lg">
          ...
          <p className="text-ink-primary text-sm leading-relaxed">{document.description}</p>
        </div>
      )}
      ...
      */}
      {hasFullText && (
        <div className="bg-surface-page border-l-4 border-border-strong p-4 rounded-r-lg">
          <button
            type="button"
          ...
          {showFullText && (
            <div className="mt-3 max-h-96 overflow-y-auto pr-2">
              <p className="text-ink-primary text-sm leading-relaxed whitespace-pre-line">
                {document.content}
              </p>
            </div>
          )}
      ...
      {tags.length > 0 && (
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {tags.map((tag, index) => (
              <span key={index} className="px-2 py-1 bg-surface-deep text-ink-secondary rounded-full text-xs font-medium">
                {tag}
              </span>
            ))}
          </div>
      ...
      <div className="pt-3 border-t border-border-subtle flex gap-2">
        ...
        {isExternalLikeDoc ? (
          <a
            ...
            className="flex-1 bg-brand-600 text-surface-page px-4 py-3 rounded-[3px] font-semibold hover:bg-brand-800 transition-all flex items-center justify-center gap-2"
          >
