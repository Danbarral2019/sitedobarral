...
export function LeiMobileDrawer({ open, onClose, children }: LeiMobileDrawerProps) {
  ...
  return (
    <div className="lg:hidden fixed inset-0 z-50">
      ...
      <div className="absolute top-0 left-0 h-full w-[80vw] max-w-sm bg-white overflow-y-auto border border-border-subtle">
        <div className="p-4 border-b border-border-subtle flex items-center justify-between sticky top-0 bg-white z-10">
          ...
          <button onClick={onClose} className="p-2 hover:bg-surface-deep rounded-[6px]">
            <X className="w-5 h-5" />
          </button>
        </div>
        {children}
