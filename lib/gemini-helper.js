 ...
 * @param {string} model - Modelo Gemini (padrão: gemini-2.5-flash)
 * @returns {Promise<string>} - Resposta do Gemini
 */
async function callGemini(prompt, model = 'gemini-3-flash-preview') {
  ...
  // IMPORTANTE: gemini-2.0-flash está deprecado pela Google (shutdown
  // 2026-06-01, ver lib/gemini/config.ts). Não existe mais "fallback
  // para 2.0" — chamadas com esse nome de modelo deixarão de responder
  // em produção. Qualquer caller que ainda passe 'gemini-2.0-flash' deve
  // ser migrado. Aqui simplesmente repassamos o model name sem remapear.
 ...
 */
async function callGeminiJSON(prompt, model = 'gemini-3-flash-preview') {
