/**
 * BIA-2 — gate de viabilidade R$0. Para cada query anotada do golden, roda o
 * retrieval de produção e verifica se algum dos top-3 resultados do tipo
 * Document é MULTI-CHUNK (>=5 chunks) — o único caso em que injetar o documento
 * inteiro (reassemblado dos chunks) acrescenta texto além do trecho recuperado.
 * Se poucas queries se qualificam, o BIA-2 é near-vacuous → NO-GO sem gastar
 * régua (Claude). Só usa embeddings Gemini + Postgres.
 */
...
async function main() {
  ...
  for (const q of queries) {
    ...
    // Top-N resultados do tipo Document (o BIA-2 só age sobre Document.content/chunks)
  ...
  console.log('');
  console.log(addressable / queries.length < 0.15
    ? '→ BIA-2 near-vacuous neste golden (<15% endereçável). NO-GO recomendado SEM gastar régua.'
    : '→ Há massa endereçável suficiente — vale medir na régua (com autorização de gasto).');
}
